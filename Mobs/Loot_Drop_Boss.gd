...
onready var orange_Raipier = preload("res://object/orangeRaipier.tscn")
onready var old_Three_Spear = preload("res://object/OldThreeSpear.tscn")
...
func _drop_loot_tier_4():
	var loot_randomizer : RandomNumberGenerator = RandomNumberGenerator.new()
	loot_randomizer.randomize()
	var loot_percent : int = loot_randomizer.randi_range(0, 300)
...
func _drop_loot_tier_5():
	...
	if loot_percent >= 0:
		for _i in range(1):
			var orange_raipier_instance = orange_Raipier.instance()
			add_child(orange_raipier_instance)
			
func _drop_loot_tier_6():
	var loot_randomizer : RandomNumberGenerator = RandomNumberGenerator.new()
	loot_randomizer.randomize()
	var loot_percent : int = loot_randomizer.randi_range(0, 5)
...
func _drop_loot_tier_7():
	...
	if loot_percent % 2 == 0:
		for _i in range(1):
			var old_three_instance = old_Three_Spear.instance()
			add_child(old_three_instance)
